...
type AnalyticsClientPageProps = {
  initialData: {
    allDailyData: DailyData[] | null;
    last7Days: Last7DayData[] | null;
...
export function AnalyticsClientPage({ initialData }: AnalyticsClientPageProps) {
  ...
  const processData = useCallback((data: AnalyticsClientPageProps['initialData']) => {
    const { allDailyData, last7Days, reservations } = data;
  ...
  const handleSync = async () => {
    setIsSyncing(true);
    // Re-fetch data on client-side for manual sync
    // This is a simplified example; in a real app you might use a server action
    try {
        ...
        // NOTE: Supabase data is not re-fetched on client sync in this example for simplicity,
        // as it requires setting up client-side Supabase instance.
        // We'll just re-process with potentially new reservation data.
        processData({
            ...initialData,
